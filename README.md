...
- 采集中心支持 RSS/指定 URL 采集源管理、立即采集、采集记录去重，并可把采集结果转为文章草稿或发布文章
...
- 部署模式支持手动发布、发布包上传、本机目录同步、宝塔 API 和 FTP/SFTP；当前版本的本机目录同步会复制到 `storage/deploy_targets/`，宝塔 API 与 FTP/SFTP 会先记录待执行任务并保留发布包
